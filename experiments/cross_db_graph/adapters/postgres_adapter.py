...
class PostgresGraphAdapter(GraphAdapter):
    ...
    def __init__(self, dsn: str, materialize: bool = False):
        self.dsn = dsn
    ...
    def query_neighbors(self, seed: str, direction: str = "out"):
        ...
        if self.materialize:
            ...
            SELECT n.*
            FROM graph_edges e
            JOIN graph_nodes n ON n.node_id = e.{target_column}
            WHERE e.{edge_column} = %s
            """
        else:
    ...
    def query_k_hop(self, seed: str, k: int, direction: str = "out"):
        ...
        if self.materialize:
            ...
            JOIN (
                ...
                FROM hop_walk
                WHERE depth > 0
            ) hops ON hops.node_id = n.node_id
            """
        else:
            sql = f"""
            WITH RECURSIVE hop_walk(node_id, depth, path) AS (
